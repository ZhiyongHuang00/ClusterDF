# -*- coding: utf-8 -*-
'''
@Time    : 2024/9/15 21:15
@Author  : HuangZhiyong
@email   : 1524338616@qq.com
@File    : Autoencoder_Gmms.py
@Function: {}:
'''
# -*- coding: utf-8 -*-
'''
...
def save_loss(loss, encoding_filterss):
    # 保存损失值到CSV文件
    loss_df = pd.DataFrame(loss, columns=['Loss'])
    loss_df.to_csv(f'D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs//{current_datetime}_{encoding_filterss}Loss.csv', index=False)
    # 绘制损失曲线D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}Number of clusters
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}_Loss.png')
    # plt.show()
    plt.close()
# 定义文件夹路径
...
# 保存到新的 .npy 文件
# x_train = np.load(r"D:\Pycharm_Project\Classification\STFT\stft_40\tmp_std_40s\500\x_train1000.npy",allow_pickle=True)
# y_train = np.load(r"D:\Pycharm_Project\Classification\STFT\stft_40\tmp_std_40s\500\y_train1000.npy",allow_pickle=True)
print(x_train.shape)

# np.save(r"D:\Pycharm_Project\Classification\STFT\stft_40\tmp_std_40s\500\x_train1000.npy", x_train)
# np.save(r"D:\Pycharm_Project\Classification\STFT\stft_40\tmp_std_40s\500\y_train1000.npy", y_train)
"""1DF、2EQ、0NS、3RF"""
...
###building model
encoder, decoder, autoencoder = Model_60s.CAE(input_shape=(64, 600, 1), filters=[12, 24, 36, 48, 64], encoding_filters=encoding_filterss,
                                            set_seed=[False, 87], summary=True)
###training
# autoencoder.compile(optimizer='adam', loss='binary_crossentropy')

autoencoder.compile(optimizer='adam', loss='mse')
csv_logger = tf.keras.callbacks.CSVLogger(save_dir + '/_gmm_pretrain_log.csv')
...
save_loss(loss_values, encoding_filterss)
...
WeightsFileName = [f'D:\Pycharm_Project\Classification\models_save//{current_datetime}_gmmPretrain_encoder.h5',
                   f'D:\Pycharm_Project\Classification\models_save//{current_datetime}_gmmPretrain_autoencoder.h5',
                   f'D:\Pycharm_Project\Classification\models_save//{current_datetime}_gmmPretrain_decoder.h5']
...
metrics_list_pretrain = []     # 初始化 metrics 列表
# features = encoder.predict(x_train)

for i in trange(2, 30):
    gmm = GaussianMixture(n_components=i, n_init=30).fit(features)
    y_pred = gmm.predict(features)  # 使用 GMM 进行预测
...
plt.yticks()
# plt.show()
plt.savefig(f"D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}_gmm Number of clusters",
            bbox_inches='tight', transparent=False)
print("Saved figures one")
# sbhzy
plt.close()
gmm = GaussianMixture(n_components = n_clusters, n_init=30).fit(features)
y_pred = gmm.predict(features)  # 使用 GMM 进行预测
...
Visualization_2Ddistribution(redu, y_true, n_clusters, mode="class", title="Pretrain Model Feature Domain by t-SNE",
                             fmt=None,
                             outfile=f"D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}pretrain_model_gmm_output")
Visualization_2Ddistribution(redu, y_pred_last, n_clusters, mode="cluster", title=None, fmt=None,
                             outfile=f"D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}pretrain_model_gmm_output_2")
# ## Model Performace
# In[10]:
import metrics
...
# 保存为 Excel 文件
error_df.to_excel(
    f'D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}_gmm_error_classification.xlsx',
    index=False)
...
plt.savefig(
    f"D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}_gmm_Pretrain model confusion matrix",
    bbox_inches='tight', transparent=False)
# plt.show()
plt.close()
# In[11]:
...
cluster_assignments(y_pred_last, n_clusters,
                    f"D:\Pycharm_Project\Classification\clustering_method\DEC\Savefig\GMMs\\{current_datetime}_gmm_cluster_assignments")
debug = 1
...
print(f"程序运行时间为 {elapsed_time:.4f} 秒")
# 打印当前时间
...
save_mode_output = {
    ...
    "encoding_filterss": encoding_filterss

}
save_name_oup = f'D:\\Pycharm_Project\\Classification\\clustering_method\\DEC\\Model_output\\GMMs\\{current_datetime}_{n_clusters}_{purity}_{encoding_filterss}GMMs_model.npz'
